...
public:
    ...
    void calcDs(fourtax &dtest, unsigned int length, int ntaxa, locusfile &file) override;
    ...
    void bootstrap(int bootstrap, std::unordered_map <std::string,int> &indlist, 
		std::default_random_engine &generator, int ntaxa, locusfile &current, std::vector<int> &keep, 
		int i, int combs, std::string *indarray, std::string output, bool hetIgnore, bool hetInclude) override;
...
private:
    ...
    void bootproc(int bootstrap, std::vector<int> &keep, double *bootD1, double *bootD2, double *bootD12, 
		  ...
		  std::default_random_engine &generator, int ntaxa, bool hetIgnore, bool hetInclude);
};
